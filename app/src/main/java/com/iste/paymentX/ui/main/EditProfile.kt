...
class EditProfile : AppCompatActivity() {
    ...
    private fun fetchUserData() {
        ...
        if (currentUser != null) {
            ...
            uidTextView.text = formatUID(userId.take(8)) // Format first 8 chars of UID
    ...
    private fun formatUID(uid: String): String {
        // Using the same improved formatting logic from UserProfile
        // Check if UID already contains formatting colons
        if (uid.contains(":")) {
            return uid.uppercase()
        }

        // If the backend sends raw characters like "9A8EB9DE", format it as "9A:8E:B9:DE"
        var formattedUID = ""
    ...
    private fun fetchUserDetailsFromBackend() {
                ...
                if (token != null) {
                    ...
                    if (response.isSuccessful && response.body() != null) {
                        ...
                        userData?.let { user ->
                            ...
                            // Update ID Card UID if available using the improved formatting
                            user.idCardUID?.let {
                                if (it.isNotEmpty()) {
                                    uidTextView.text = formatUID(it)
                                }
                            }
    ...
    private fun navigateToUpdatePhoneNumber() {
        val intent = Intent(this, UpdatePhoneNumber::class.java)
