...
import com.iste.paymentx.ui.auth.ConfirmPIN

class MerchantCreateTransPIN : AppCompatActivity() {
    // store credientials
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_merchant_create_trans_pin)
        ...
        val pinInputs = listOf(
            ...
            findViewById<EditText>(R.id.merchuid_input6)
        )
        ...
        val verifyButton = findViewById<Button>(R.id.merchbtnVerify)
        verifyButton.setOnClickListener {
            // Ensure all inputs are filled
            val pin = pinInputs.joinToString("") { it.text.toString() }
            if (pin.length == pinInputs.size) {
                val intent = Intent(this, MerchantConfirmPIN::class.java).apply {
